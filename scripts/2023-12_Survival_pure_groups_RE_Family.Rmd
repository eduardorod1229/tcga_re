...
oligo_df[,retroelements_all] <- scale(oligo_df[,retroelements_all], center=FALSE, scale=TRUE)
cols_miss <- colSums(is.na(oligo_df)) == nrow(oligo_df)
oligo_df <- oligo_df[, !cols_miss]
retroelements <- c(names(oligo_df[,2:50]))
```
...
suppressWarnings({for (re in retroelements) { 
  ...
  re_median <- median(oligo_df[[re]])
  p_value <- summary(surv_model)$coefficients[35]
  
  
  if (!is.na(p_value) && p_value <= 0.051 && re_median >.5) {
...
```{r}
astro_df[,retroelements_all] <- scale(astro_df[,retroelements_all], center=FALSE, scale=TRUE)
cols_miss <- colSums(is.na(astro_df)) == nrow(astro_df)
astro_df <- astro_df[, !cols_miss]
retroelements <- c(names(astro_df[,2:50]))
...
```{r}
gbm_df[,retroelements_all] <- scale(gbm_df[,retroelements_all], center=FALSE, scale=TRUE)
cols_miss <- colSums(is.na(gbm_df)) == nrow(gbm_df)
gbm_df <- gbm_df[, !cols_miss]
retroelements <- c(names(gbm_df[,2:49]))
...
suppressWarnings({for (re in retroelements) { 
  ...
  re_median <- median(gbm_df[[re]])
  p_value <- summary(surv_model)$coefficients[30]
  
  if (!is.na(p_value) && p_value <= 0.051 && re_median >0.5) {
        coef <- summary(surv_model)$coefficients[6]
        exp_coef <- summary(surv_model)$coefficients[12]
        se_coef <- summary(surv_model)$coefficients[18]
        z <- summary(surv_model)$coefficients[24]
...
```{r}
glionec_df[,retroelements_all] <- scale(glionec_df[,retroelements_all], center=FALSE, scale=TRUE)
cols_miss <- colSums(is.na(glionec_df)) == nrow(glionec_df)
glionec_df <- glionec_df[, !cols_miss]
retroelements <- c(names(glionec_df[,2:49]))
...
suppressWarnings({for (re in retroelements) { 
  ...
  re_median <- median(glionec_df[[re]])
  p_value <- summary(surv_model)$coefficients[30]
  
  
  if (!is.na(p_value) && p_value <= 0.1 && re_median >0) {
        coef <- summary(surv_model)$coefficients[6]
        exp_coef <- summary(surv_model)$coefficients[12]
        se_coef <- summary(surv_model)$coefficients[18]
        z <- summary(surv_model)$coefficients[24]
