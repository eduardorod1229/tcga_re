---
title: '2023-12 Survival by "Pure" RE All'
subtitle: RE project
output:
  html_document:
    df_print: paged
...
retroelements <- c(names(oligo_df[,2:5050]))
```
...
suppressWarnings({for (re in retroelements) { 
  ...
  if (!is.na(p_value) && p_value <= 0.01 && re_median >0) {
...
retroelements <- c(names(astro_df[,2:5282]))
```
...
retroelements <- c(names(gbm_df[,2:3360]))
```
...
retroelements <- c(names(glionec_df[,2:4493]))
```
...
suppressWarnings({for (re in retroelements) { 
  ...
  if (!is.na(p_value) && p_value <= 0.051 && re_median >0) {
